              ...
              double pmissx, double pmissy)
{
  solved = false;     //reset solved tag when momenta are changed.
   ...
   Ea   = sqrt(Easq);
   masq = std::max(0., Easq - pax*pax - pay*pay);
   ma   = sqrt(masq);
   ...
   Eb   = sqrt(Ebsq);
   mbsq = std::max(0., Ebsq - pbx*pbx - pby*pby);
   mb   = sqrt(mbsq);
...
//   cout << std::endl << "ma+mn1 = " << ma + mn1 << std::endl;
...
//void mt2bl::print()
//{
//   cout << " pax = " << pax*scale << ";   pay = " << pay*scale << ";   ma = " << ma*scale <<";"<< std::endl;
//   cout << " pbx = " << pbx*scale << ";   pby = " << pby*scale << ";   mb = " << mb*scale <<";"<< std::endl;
//   cout << " pmissx = " << pmissx*scale << ";   pmissy = " << pmissy*scale <<";"<< std::endl;
//   cout << " mn = " << mn_unscale<<";" << std::endl;
//}
...
//cout << "pmissx = " << pmissx << "  ,  pmissy = " << pmissy << std::endl;
//cout << "ma = " << ma  << "  ,  mb = " << mb <<endl;
//cout << "a2 = " << a2   << " ,  d2=" << d2  << " ,  e2=" << e2  << " ,  f2=" << f2 << std::endl;
...
//if nsols_high=4, also need a scan because we may find the wrong tangent point.
   ...
   //   foundhigh = find_high(Deltasq_high);
      if(foundhigh == 0)
      {
   cout << "Deltasq_high not found, output ma+mn1" << std::endl;
//      cout << "myhigh1 = " << sqrt(Deltasq_high1 + mn1sq) << std::endl;
//      cout << "myhigh2 = " << sqrt(Deltasq_high2 + mn1sq) << std::endl;
//      cout << "nsol(high1) = " << nsols(Deltasq_high1) << std::endl;
//      cout << "nsol(high2) = " << nsols(Deltasq_high2) << std::endl;
...
int mt2bl::scan_high(double & Deltasq_high)
{
   ...
   //if (nevt == 32334) cout << "Deltasq_high = " << Deltasq_high << std::endl;
