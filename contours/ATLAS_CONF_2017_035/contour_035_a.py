...
import os
shit = os.walk("/scratch18/kazuki2/result/035_a") ## Directorio no q vai fozar
craps = shit.next()
i = 0
x = np.array([])
y = np.array([])
...
for crap in craps[2]: ## o terceiro elmento da lista son os arquivos
    if 'ans' in crap: ##marqueinos coa extension ans para identificalos
        ...
        if x0 >= (y0):
             ...
             x = np.append(x, x0)
             y = np.append(y, y0)
             dc = cPickle.load(file("/scratch18/kazuki2/result/035_a/"+crap))
...
plt.title('C1lL3lN1_C1lL3lN1 ATLAS_CONF_2017_035')
plt.axis([100, 750, 0, 450])
plt.grid(True)
plt.show()
