...
import os
shit = os.walk("/scratch18/kazuki2/result/1602_09058_b") ## Directorio no q vai fozar
craps = shit.next()
i = 0
x = np.array([])
y = np.array([])
...
for crap in craps[2]: ## o terceiro elmento da lista son os arquivos
    if 'ans' in crap: ##marqueinos coa extension ans para identificalos
        ...
        x = np.append(x, x0)
        y = np.append(y, y0)
        dc = cPickle.load(file("/scratch18/kazuki2/result/1602_09058_b/"+crap))
...
plt.axis([700, 1400, 100, 1000])
plt.xlabel('Gluino mass (GeV)')
plt.ylabel('Neutralino mass (GeV)')
plt.title('1602.9058 (b) GqqC1wN2zN1')
plt.grid(True)
plt.show()

q = open('1602_09058_b.txt', 'w')
Pt = Prob.transpose()
