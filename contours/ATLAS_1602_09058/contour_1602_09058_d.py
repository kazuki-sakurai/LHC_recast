...
import os
shit = os.walk("/scratch18/kazuki2/result/1602_09058_d") ## Directorio no q vai fozar
craps = shit.next()
i = 0
x = np.array([])
y = np.array([])
...
for crap in craps[2]: ## o terceiro elmento da lista son os arquivos
    if 'ans' in crap: ##marqueinos coa extension ans para identificalos
        ...
        if x0 >= y0:
             x = np.append(x, x0)
             y = np.append(y, y0)
             dc = cPickle.load(file("/scratch18/kazuki2/result/1602_09058_d/"+crap))
...
plt.title('1602.09058 (d) GttN1')
plt.grid(True)
plt.axis([850, 1430, 100, 1100])
plt.show()

q = open('1602_09058_d.txt', 'w')
Pt = Prob.transpose()
