...
import os
shit = os.walk("/scratch18/kazuki2/result/1602_09058_a") ## Directorio no q vai fozar
craps = shit.next()
i = 0
x = np.array([])
y = np.array([])
...
for crap in craps[2]: ## o terceiro elmento da lista son os arquivos
    if 'ans' in crap: ##marqueinos coa extension ans para identificalos
        ...
        if x0 >= y0:
             x = np.append(x, x0)
             y = np.append(y, y0)
             dc = cPickle.load(file("/scratch18/kazuki2/result/1602_09058_a/"+crap))
...
plt.title('1602.09058 (a) GqqN2lLlN1')
plt.grid(True)
plt.axis([500, 1600, 100, 1400])
plt.show()

q = open('1602_09058_a.txt', 'w')
Pt = Prob.transpose()
