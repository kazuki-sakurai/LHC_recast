...
import os
shit = os.walk("/scratch18/kazuki2/result/1602_09058_c2/") ## Directorio no q vai fozar
craps = shit.next()
i = 0
x = np.array([])
y = np.array([])
...
for crap in craps[2]: ## o terceiro elmento da lista son os arquivos
    if 'ans' in crap: ##marqueinos coa extension ans para identificalos
        ...
        x = np.append(x, x0)
        y = np.append(y, y0)
        dc = cPickle.load(file("/scratch18/kazuki2/result/1602_09058_c2/"+crap))
        z = dc['atlas_1602_09058']['chi2_i']
        sigPos = dc['atlas_1602_09058']['chi2_i_+1sig']
        sigNeg = dc['atlas_1602_09058']['chi2_i_-1sig']
        P = 1 - st.chi2.cdf(z,1)
...
plt.grid(True)
plt.axis([400, 700, 0, 350])
plt.xlabel('b mass (GeV)')
plt.ylabel('Nreutralino mass (GeV)')
plt.title('1602.09058 (c) B1tC1wN1')
plt.show()

q = open('1602_09058_c.txt', 'w')
Pt = Prob.transpose()
