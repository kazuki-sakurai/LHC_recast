...
import os
shit = os.walk("/scratch18/kazuki2/result/039_c") ## Directorio no q vai fozar
craps = shit.next()
i = 0
x = np.array([])
y = np.array([])
...
for crap in craps[2]: ## o terceiro elmento da lista son os arquivos
    if 'ansc' in crap: ##marqueinos coa extension ans para identificalos
        names = crap.split("_")
        x0 = float(names[1])
        y0 = float(names[2].replace('.eff', ''))
        ...
        if x0 >= (y0):
             ...
             x = np.append(x, x0)
             y = np.append(y, y0)
             dc = cPickle.load(file("/scratch18/kazuki2/result/039_c/"+crap))
             z = dc['atlas_conf_2017_039_c']['chi2_ic']
             sigPos = dc['atlas_conf_2017_039_c']['chi2_i_+1sigc']
             sigNeg = dc['atlas_conf_2017_039_c']['chi2_i_-1sigc']
             P = 1 - st.chi2.cdf(z, 15)
             Ppos = 1 - st.chi2.cdf(sigPos, 15)
             Pneg = 1 - st.chi2.cdf(sigNeg, 15)
             f[(x0,y0)] = P
             fpos[(x0, y0)] = Ppos
             fneg[(x0, y0)] = Pneg
...
plt.xlabel('slepton mass (GeV)')
plt.ylabel('Neutralino mass (GeV)')
plt.title('LlN1 ATLAS_CONF_2017_039')
plt.axis([100, 650, 0, 450])
plt.grid(True)
plt.show()

## q = open('atlas_conf_2017_039_c.txt', 'w')
## Pt = Prob.transpose()
