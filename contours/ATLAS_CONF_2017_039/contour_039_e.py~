...
import os
shit = os.walk("/scratch18/kazuki2/result/039_e") ## Directorio no q vai fozar
craps = shit.next()
i = 0
x = np.array([])
y = np.array([])
...
for crap in craps[2]: ## o terceiro elmento da lista son os arquivos
    if 'anse' in crap: ##marqueinos coa extension ans para identificalos
        names = crap.split("_")
        x0 = float(names[2])
        ...
        if x0 >= (y0):
             ...
             x = np.append(x, x0)
             y = np.append(y, y0)
             dc = cPickle.load(file("/scratch18/kazuki2/result/039_e/"+crap))
             z = dc['atlas_conf_2017_039_e']['chi2_ie']
             sigPos = dc['atlas_conf_2017_039_e']['chi2_i_+1sige']
             sigNeg = dc['atlas_conf_2017_039_e']['chi2_i_-1sige']
             P = 1 - st.chi2.cdf(z,6)
             Ppos = 1 - st.chi2.cdf(sigPos, 6)
             Pneg = 1 - st.chi2.cdf(sigNeg, 6)
             f[(x0,y0)] = P
             fpos[(x0, y0)] = Ppos
             fneg[(x0, y0)] = Pneg
...
plt.title('C1whadN1_N2zlepN1 ATLAS_CONF_2017_039')
plt.axis([100, 700, 0, 500])
plt.grid(True)
plt.show()

## q = open('atlas_conf_2017_039_e.txt', 'w')
## Pt = Prob.transpose()
