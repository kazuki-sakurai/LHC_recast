...
import os
shit = os.walk("/scratch18/kazuki2/result/039_d/") ## Directorio no q vai fozar
craps = shit.next()
i = 0
x = np.array([])
y = np.array([])
...
for crap in craps[2]: ## o terceiro elmento da lista son os arquivos
    if 'ans' in crap: ##marqueinos coa extension ans para identificalos
        ...
        if x0 >= (y0):
             ...
             x = np.append(x, x0)
             y = np.append(y, y0)
             dc = cPickle.load(file("/scratch18/kazuki2/result/039_d/"+crap))
             z = dc['atlas_conf_2017_039_d']['chi2_id']
             sigPos = dc['atlas_conf_2017_039_d']['chi2_i_+1sigd']
             sigNeg = dc['atlas_conf_2017_039_d']['chi2_i_-1sigd']
             P = 1 - st.chi2.cdf(z, 1)
...
plt.title('C1wlepN1_N2zlepN1 ATLAS_CONF_2017_039')
plt.axis([100, 700, 0, 500])
plt.grid(True)
plt.show()

q = open('atlas_conf_2017_039_d.txt', 'w')
Pt = Prob.transpose()
