...
import os
shit = os.walk("/scratch18/kazuki2/result/039_b_2") ## Directorio no q vai fozar
craps = shit.next()
i = 0
x = np.array([])
y = np.array([])
...
for crap in craps[2]: ## o terceiro elmento da lista son os arquivos
    if 'ans' in crap: ##marqueinos coa extension ans para identificalos
        ...
        if x0 >= (y0):
             ...
             x = np.append(x, x0)
             y = np.append(y, y0)
             dc = cPickle.load(file("/scratch18/kazuki2/result/039_b_2/"+crap))
             z = dc['atlas_conf_2017_039_b']['chi2_ib']
             sigPos = dc['atlas_conf_2017_039_b']['chi2_i_+1sigb']
             sigNeg = dc['atlas_conf_2017_039_b']['chi2_i_-1sigb']
             P = 1 - st.chi2.cdf(z,5)
             Ppos = 1 - st.chi2.cdf(sigPos, 5)
             Pneg = 1 - st.chi2.cdf(sigNeg, 5)
             f[(x0,y0)] = P
             fpos[(x0, y0)] = Ppos
             fneg[(x0, y0)] = Pneg
...
plt.title('C1lLlN1_N2lLlN1 ATLAS_CONF_2017_039')
plt.axis([200, 1400, 200, 1200])
plt.grid(True)
plt.show()

q = open('atlas_conf_2017_039_b.txt', 'w')
Pt = Prob.transpose()
