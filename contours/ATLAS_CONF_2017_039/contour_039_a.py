...
import os
shit = os.walk("/scratch18/kazuki2/result/039_a_4") ## Directorio no q vai fozar
craps = shit.next()
i = 0
x = np.array([])
y = np.array([])
...
for crap in craps[2]: ## o terceiro elmento da lista son os arquivos
    if 'ans' in crap: ##marqueinos coa extension ans para identificalos
        ...
        if x0 >= (y0):
             ...
             x = np.append(x, x0)
             y = np.append(y, y0)
             dc = cPickle.load(file("/scratch18/kazuki2/result/039_a_4/"+crap))
             z = dc['atlas_conf_2017_039_a']['chi2_ia']
             sigPos = dc['atlas_conf_2017_039_a']['chi2_i_+1siga']
             sigNeg = dc['atlas_conf_2017_039_a']['chi2_i_-1siga']
             P = 1 - st.chi2.cdf(z,23)
             Ppos = 1 - st.chi2.cdf(sigPos, 23)
             Pneg = 1 - st.chi2.cdf(sigNeg, 23)
             f[(x0,y0)] = P
             fpos[(x0, y0)] = Ppos
             fneg[(x0, y0)] = Pneg
...
plt.title('C1lLlN1_C1lLlN1 ATLAS_CONF_2017_039')
plt.axis([100, 900, 0, 600])
plt.grid(True)
plt.show()

q = open('atlas_conf_2017_039_a.txt', 'w')
Pt = Prob.transpose()
