...
import os
shit = os.walk("/scratch18/kazuki2/result/054_GG") ## Directorio no q vai fozar
craps = shit.next()
i = 0
x = np.array([])
y = np.array([])
...
for crap in craps[2]: ## o terceiro elmento da lista son os arquivos
    if 'ans' in crap: ##marqueinos coa extension ans para identificalos
        ...
        x = np.append(x, x0)
        y = np.append(y, y0)
        dc = cPickle.load(file("/scratch18/kazuki2/result/054_GG/"+crap))
        z = dc['atlas_conf_2016_054']['chi2_i']
        P = 1 - st.chi2.cdf(z,1)
        f[(x0,y0)] = P
...
plt.title('ATLAS CONF NOTE 054 (all SR)')
plt.axis([200, 2000, 100, 1400])
plt.show()
