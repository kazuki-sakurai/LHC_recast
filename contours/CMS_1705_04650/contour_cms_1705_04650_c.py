...
import os
shit = os.walk("/scratch28/isabel.suarez/kazuki2/result/cms_1705_04650_c") ## Directorio no q vai fozar
craps = shit.next()
i = 0
x = np.array([])
y = np.array([])
...
for crap in craps[2]: ## o terceiro elmento da lista son os arquivos
    if 'ans' in crap: ##marqueinos coa extension ans para identificalos
        ...
        if x0 >= (y0):
             ...
             x = np.append(x, x0)
             y = np.append(y, y0)
             dc = cPickle.load(file("/scratch28/isabel.suarez/kazuki2/result/cms_1705_04650_c/"+crap))
...
plt.title('GqqN1 CMS_1705_04650 ')
plt.axis([600, 2300, 0, 1600])
plt.grid(True)
plt.show()

q = open('cms_1705_04650_c.txt', 'w')
Pt = Prob.transpose()
