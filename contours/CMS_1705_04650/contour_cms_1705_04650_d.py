...
import os
shit = os.walk("/scratch18/kazuki2/result/cms_d") ## Directorio no q vai fozar
craps = shit.next()
i = 0
x = np.array([])
y = np.array([])
...
for crap in craps[2]: ## o terceiro elmento da lista son os arquivos
    if 'ans' in crap: ##marqueinos coa extension ans para identificalos
        ...
        if x0 >= (y0):
             ...
             x = np.append(x, x0)
             y = np.append(y, y0)
             dc = cPickle.load(file("/scratch18/kazuki2/result/cms_d/"+crap))
...
plt.xlabel('Bottom squark mass (GeV)')
plt.ylabel('Neutralino mass (GeV)')
plt.title('B1tN1 CMS_1705_04650 ')
plt.axis([300, 1300, 0, 700])
plt.grid(True)
plt.show()

q = open('cms_1705_04650_d.txt', 'w')
Pt = Prob.transpose()
