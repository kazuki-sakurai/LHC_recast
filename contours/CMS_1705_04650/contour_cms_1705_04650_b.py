...
import os
shit = os.walk("/scratch18/kazuki2/result/cms_1705_04650_4") ## Directorio no q vai fozar
craps = shit.next()
i = 0
x = np.array([])
y = np.array([])
...
for crap in craps[2]: ## o terceiro elmento da lista son os arquivos
    if 'ans' in crap: ##marqueinos coa extension ans para identificalos
        ...
        if x0 >= (y0):
             ...
             x = np.append(x, x0)
             y = np.append(y, y0)
             dc = cPickle.load(file("/scratch18/kazuki2/result/cms_1705_04650_4/"+crap))
...
plt.title('GttN1 CMS_1705_04650 ')
plt.axis([600, 2200, 0, 1800])
plt.grid(True)
plt.show()

q = open('cms_1705_04650_b.txt', 'w')
Pt = Prob.transpose()
