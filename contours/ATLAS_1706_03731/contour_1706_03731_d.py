...
import os
shit = os.walk("/scratch27/isabel.suarez/kazuki2/result/1706_03731_d") ## Directorio no q vai fozar
craps = shit.next()
i = 0
x = np.array([])
y = np.array([])
...
for crap in craps[2]: ## o terceiro elmento da lista son os arquivos
    if 'ans' in crap: ##marqueinos coa extension ans para identificalos
        ...
        if x0 >= (y0):
             ...
             x = np.append(x, x0)
             y = np.append(y, y0)
             dc = cPickle.load(file("/scratch27/isabel.suarez/kazuki2/result/1706_03731_d/"+crap))
             z = dc['atlas_1706_03731']['chi2_i']
             sigPos = dc['atlas_1706_03731']['chi2_i_+1sig']
             sigNeg = dc['atlas_1706_03731']['chi2_i_-1sig']
             P = 1 - st.chi2.cdf(z,1)
...
plt.title('GqqN2lLlN1 1706.03731 ')
plt.axis([600, 2200, 200, 2200])
plt.grid(True)
plt.show()

q = open('1706_03731_d.txt', 'w')
Pt = Prob.transpose()
