...
import os
shit = os.walk("/scratch18/kazuki2/result/037_c6/") ## Directorio no q vai fozar
craps = shit.next()
i = 0
x = np.array([])
y = np.array([])
...
for crap in craps[2]: ## o terceiro elmento da lista son os arquivos
    if 'ans' in crap: ##marqueinos coa extension ans para identificalos
        ...
        if x0 >= (y0 + 272):
             x = np.append(x, x0)
             y = np.append(y, y0)
             dc = cPickle.load(file("/scratch18/kazuki2/result/037_c6/"+crap))
...
plt.xlabel('sbottom mass (GeV)')
plt.ylabel('Neutralino mass (GeV)')
plt.title('037 (c) B1tC1wN1')
plt.axis([400, 850, 50, 600])
plt.grid(True)
plt.show()
