...
import os
shit = os.walk("/scratch18/kazuki2/result/Grid1") ## Directorio no q vai fozar
craps = shit.next()
i = 0
x = np.array([])
y = np.array([])
...
for crap in craps[2]: ## o terceiro elmento da lista son os arquivos
    if 'ans' in crap: ##marqueinos coa extension ans para identificalos
        ...
        x = np.append(x, x0)
        y = np.append(y, y0)
        dc = cPickle.load(file("/scratch18/kazuki2/result/Grid1/"+crap))
        z =  dc['atlas_1602_09058']['chi2_i_-1sig']
        ## for key in dc.keys():
        ##      A += dc[key]['chi2_i']
        ##      z = A
   #z = dc['chi2glob']
